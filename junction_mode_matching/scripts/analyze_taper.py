...
def main():
    ...
    for ax, L in zip(axes[0], Ls):
        ...
        # у контроля без перехода адиабатический эталон вырождается в участок без металла - столбик не рисуется
        ax.bar(x + 1.5 * wdt, [-10 * math.log10(t[6]) if (t[6] == t[6] and t[0] not in ("step", "butt")) else 0 for t in rows_L], wdt, label="адиабатический эталон exp(−∫α dz)", color="#9467bd")
        ax.set_xticks(x)
        ax.set_xticklabels(names, rotation=30, ha="right", fontsize=8)
        ax.set_ylabel("потери перехода, дБ")
        ax.set_title("длина перехода L = %.2f мкм" % L, fontsize=10)
