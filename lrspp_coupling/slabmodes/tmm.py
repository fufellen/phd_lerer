...
def mode_fields(
    ...
    # Нормировка. Длины здесь в микрометрах, поэтому удобно ввести
    #     E~ = (omega eps0 / 1e6) E,   H~ = H_y,
    # тогда обе компоненты E выражаются через H_y без размерных множителей:
    #     E~_x = (1 / (i eps)) dH_y/dz,   E~_z = -(k0 n_eff / eps) H_y,
    # где производная и k0 берутся в мкм^-1. В этих переменных уравнения
    # Максвелла принимают вид rot E~ = i k0^2 H~ и rot H~ = -i eps E~.
    ex = ex / 1j
    ez = -(k0 * neff) * hy / eps_z
    # Множитель k0 у E_z обязателен: v = (1/eps) dH_y/dz уже несёт k0 через
    # производную, а -n_eff H_y / eps - нет. Без него E_z занижена в k0 раз,
    # то есть в 4.05 раза при 1.55 мкм, и восстановленный вектор E повёрнут.
    # Проверяется невязкой уравнений Максвелла в scripts/selftest.py.
    return hy, ex, ez
