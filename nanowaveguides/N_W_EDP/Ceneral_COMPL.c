	...
	*******************/
	...
	for ( m=1; m <= M; m++ )
	{
		...
		for ( n=1; n <= Nsl+1; n++ )
		{
			...
			if (!hru)
			{
				if (EPS_MAX < fabs(EPS_r[n]))
					EPS_MAX = fabs(EPS_r[n]);

				if (EPS_min > fabs(EPS_r[n]))
					EPS_min = fabs(EPS_r[n]);
			}
		}
	...
	{	
		...
		ik = 0;
		bet_i[i] = z[2] = DK[1][i] - DK[2][i];
		z[0] = DK[2][i] + DK[3][i] - 2.0 * DK[1][i];
		bet_r[i] = z[1] = DK[1][i] - z[0] * 0.5;
